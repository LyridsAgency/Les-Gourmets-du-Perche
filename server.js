...
function empreinte(motDePasse, selHex, iterations) {
  return crypto.pbkdf2Sync(motDePasse, Buffer.from(selHex, "hex"), iterations, 32, "sha256");
}

function verifierMotDePasse(motDePasse, securite) {
  if (!securite || !securite.hash) return true; // pas encore configuré
  const calculee = empreinte(String(motDePasse), securite.sel, securite.iterations);
...
app.post("/api/connexion", (req, res) => {
  ...
  if (!verifierMotDePasse(req.body && req.body.motDePasse, contenu.securite)) {
    noterEchec(ip);
...
app.post("/api/admin/mot-de-passe", exigerConnexion, (req, res) => {
  const { actuel, nouveau } = req.body || {};
  const contenu = lireContenu();
  if (!verifierMotDePasse(actuel, contenu.securite)) {
  ...
  contenu.securite = {
    sel,
    hash: empreinte(nouveau, sel, ITERATIONS_MDP).toString("hex"),
